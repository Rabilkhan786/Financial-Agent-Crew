"""Fetch recent company news and optional news sentiment."""
...
def _fetch_yahoo(ticker, limit):
    """Fetch Yahoo Finance headlines in its old or new response shape."""
...
def _fetch_alphavantage_sentiment(ticker, api_key):
    """Return relevance-weighted Alpha Vantage news sentiment."""
    query = urllib.parse.urlencode(
        {
...
def fetch_news(ticker, limit=DEFAULT_LIMIT, days=DEFAULT_DAYS, use_cache=True):
    """Return recent articles and optional sentiment without failing the run."""
    ...
    def produce():
        ...
        if alphavantage_key:
            try:
                sentiment = _fetch_alphavantage_sentiment(
                    ticker,
                    alphavantage_key,
                )
            except Exception as error:
    ...
    try:
        if use_cache:
            result, data_source = cache.cached(
                "news",
                key,
                produce,
